...
namespace PetVision.Web.Controllers
{
    public class QuoteController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            if(predictedBreed.TagName == "Regdoll")
            {
                predictedBreed.TagName = "Ragdoll";
            }

            using (var ctx = new ClaimDataContext())
            {
                List<PetCondition> claimData = null;
                try
                {
                    claimData = ctx.Conditions.Where(x => x.Breed == predictedBreed.TagName.ToUpper() && x.State == petState.ToString()).OrderBy(x => x.Rank).Take(3).Select(x => new PetCondition
                    {
                ...
                {
                    claimData = ctx.Conditions.Where(x => x.Breed == predictedBreed.TagName.ToUpper()).OrderBy(x => x.Rank).Take(3).Select(x => new PetCondition
                    {
                        ClaimAmount = x.ClaimAmount,
                        Condition = x.DiagnosisCodeDesc,
                        PaidAmount = x.PaidAmount,
