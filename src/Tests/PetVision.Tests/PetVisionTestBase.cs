...
using PetVision.Migrations.SeedData;
using PetVision.MultiTenancy;
using PetVision.Users;
using Castle.MicroKernel.Registration;
using Effort;
using EntityFramework.DynamicFilters;

namespace PetVision.Tests
{
    public abstract class PetVisionTestBase : AbpIntegratedTestBase<PetVisionTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected PetVisionTestBase()
        {
        ...
        protected void UsingDbContext(Action<PetVisionDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PetVisionDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PetVisionDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PetVisionDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PetVisionDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PetVisionDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PetVisionDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PetVisionDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PetVisionDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PetVisionDbContext, Task<T>> func)
        {
            T result;
